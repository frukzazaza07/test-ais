<template>
  <div>
    ...
    <v-data-table v-bind="{ ...dataTableDefault, ...$props.dataTable }">
      ...
      <template
        v-for="val of $props.dataTable?.headers"
        v-slot:[`item.${val.key}`]="{ item, internalItem, index }"
      >
        <slot
          v-if="$slots[`item.${val.key}`]"
          :name="`item.${val.key}`"
          :item="item"
          :index="index"
        ></slot>
    ...
    <div class="d-flex justify-end">
      <v-pagination
        v-bind="{ ...paginateDefault, ...$props.paginate }"
        :model-value="currentPage"
        @update:model-value="(v) => $emit('update:currentPage', v)"
      ></v-pagination>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {},
  watch: {},
  emits: ['update:search', 'update:currentPage', 'update:submitSearch'],
  async mounted() {},
  methods: {},
}
</script>
